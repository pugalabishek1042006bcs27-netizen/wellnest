...
const MAX_STORY_TEXT_LENGTH = 120
...
const Blog = () => {
  ...
  const [storyDurationSeconds, setStoryDurationSeconds] = useState(0)
  const [commentInputs, setCommentInputs] = useState({})
  const storyComposerInputRef = useRef(null)
  ...
  const handleDeleteStory = (storyId) => {
    ...
    setStories((prev) => {
      const nextStories = prev.filter((story) => story.id !== storyId)
      writeStorage(BLOG_STORIES_KEY, nextStories)
      return nextStories
    })

    setActiveStory((prev) => (prev?.id === storyId ? null : prev))
  }

  const handleSelectStoryVideo = async (event) => {
    ...
    try {
      ...
      setStoryPreviewUrl(videoDataUrl)
      setStoryDurationSeconds(Math.round(duration))
    } catch {
  ...
  const handleUploadStory = async () => {
    ...
    try {
      ...
      const createdStory = {
        ...
        visibility: 'friends',
        videoUrl: storyPreviewUrl,
        textOverlay: trimmedStoryText,
  ...
  return (
    <div className="blog-page">
        ...
        <section className="stories-section">
          ...
          <div className="stories-row">
            ...
            {orderedStories.map((story) => (
              <article key={story.id} className="story-chip">
                <div className="story-chip-top">
        ...
        <section className="posts-section">
          ...
          <div className="posts-list">
            {orderedPosts.map((post) => (
              <article key={post.id || post._id} className="post-card">
      ...
      <button type="button" className="create-post-fab" onClick={openCreatePostModal}>
        +
      </button>
        ...
        <div className="blog-modal-overlay" onClick={closePostModal}>
          <div className="blog-modal" onClick={(event) => event.stopPropagation()}>
            <div className="blog-modal-head">
              ...
              <button type="button" onClick={closePostModal}>x</button>
            </div>
            ...
            <div className="blog-modal-actions">
              <button type="button" className="secondary" onClick={closePostModal}>Cancel</button>
              <button type="button" className="primary" onClick={handleSubmitPost} disabled={isSavingPost}>
                {isSavingPost ? 'Saving...' : 'Publish'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="story-viewer-overlay" onClick={() => setActiveStory(null)}>
          <div className="story-viewer" onClick={(event) => event.stopPropagation()}>
            <div className="story-viewer-head">
              ...
              <div className="story-viewer-head-actions">
                {(activeStory.authorEmail || '') === (user?.email || '') && (
                  <button
                    type="button"
            ...
            <div className="story-video-wrap">
              ...
              {activeStory.textOverlay && (
                <div className="story-video-text-overlay">{activeStory.textOverlay}</div>
              )}
            </div>
      ...
      {isStoryComposerOpen && (
        <div className="story-viewer-overlay" onClick={closeStoryComposer}>
          <div className="story-composer" onClick={(event) => event.stopPropagation()}>
            <div className="story-viewer-head">
              <span>Create Story</span>
              <button type="button" onClick={closeStoryComposer}>x</button>
            </div>
            ...
            <div className="story-composer-body">
              {storyPreviewUrl ? (
                ...
                <div className="story-preview-placeholder">
                  <p>Select a video to preview your story.</p>
                </div>
              )}

              <div className="story-text-wrap">
                <label htmlFor="story-text-input">Text over video</label>
                <input
                  id="story-text-input"
                  type="text"
                  value={storyText}
            ...
            <div className="story-composer-actions">
              ...
              <button
                type="button"
                className="secondary"
                onClick={() => storyComposerInputRef.current?.click()}
                disabled={isCreatingStory}
              >
                Choose Video
              </button>
              <button
                ...
                disabled={isCreatingStory || !storyPreviewUrl}
              >
                {isCreatingStory ? 'Uploading...' : 'Upload Story'}
              </button>
            </div>

            <input
              ref={storyComposerInputRef}
              type="file"
              accept="video/mp4,video/webm,video/quicktime"
              onChange={handleSelectStoryVideo}
              hidden
            />
          </div>
        </div>
