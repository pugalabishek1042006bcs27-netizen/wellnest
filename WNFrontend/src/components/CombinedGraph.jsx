...
const METRIC_OPTIONS = ['all', 'sleep', 'calories', 'water', 'workout']
...
const CombinedGraph = () => {
  ...
  const selectedMetricSeries = selectedMetric === 'all' ? null : series[selectedMetric]
  const allMetricKeys = Object.keys(METRIC_CONFIG)
  const barDatasets = selectedMetric === 'all'
    ? allMetricKeys.map((metricKey) => ({
        label: METRIC_CONFIG[metricKey].label,
        data: series[metricKey],
        backgroundColor: METRIC_CONFIG[metricKey].background,
    ...
    <div className="home-container">
      ...
      <div className="container">
              ...
              >
                {METRIC_OPTIONS.map((metric) => (
                  <option key={metric} value={metric}>
                    {metric === 'all' ? 'All Parameters' : METRIC_CONFIG[metric].label}
                  </option>
                ))}
              </select>
            </div>
        ...
        <section className="section-card animate delay-5">
          <div className="chart-card" style={{ maxWidth: '920px', margin: '0 auto' }}>
            <div className="chart-title">{selectedMetric === 'all' ? `All Parameters (${periodTitle[period]}) (Bar)` : `${selectedMetricConfig.label} (${periodTitle[period]}) (Bar)`}</div>
            <div className="chart-subtitle">{selectedMetric === 'all' ? 'Displays workout, water, sleep, and calories together in one grouped bar chart.' : 'Switch metric from the dropdown to focus on one chart.'}</div>
