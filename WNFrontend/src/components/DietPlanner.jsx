  ...
  const renderDietPlan = (rawPlan) => {
    ...
    const isHeading = (line) => {
      ...
      return (
        /^\*\*[^*]+\*\*\s*:?\s*$/.test(t) ||
        /^#+\s+/.test(t) ||
        /^(monday|tuesday|wednesday|thursday|friday|saturday|sunday|day\s*\d+|week\s*\d+|breakfast|lunch|dinner|snack|warm[\s-]*up|cool[\s-]*down|main workout|overview|notes?|tips?|summary|weekly|daily|introduction|conclusion|duration|total|nutrition|hydration|general)\s*:?\s*$/i.test(t) ||
        /^(monday|tuesday|wednesday|thursday|friday|saturday|sunday|day\s*\d+|week\s*\d+)\s*:/i.test(t)
      )
    }

    const isMealLine = (line) => {
      return /^\s*(breakfast|lunch|dinner|snack)\s*:/i.test(line)
    }
    ...
    lines.forEach((line, i) => {
      ...
      if (isHeading(trimmed)) {
        ...
        // Top-level day headings
        if (/^(monday|tuesday|wednesday|thursday|friday|saturday|sunday|day\s*\d+|week\s*\d+)/i.test(trimmed)) {
          elements.push(
            <h4 key={key++} className="plan-day-heading">{text}</h4>
          )
        } else if (/^(breakfast|lunch|dinner|snack)/i.test(trimmed)) {
          elements.push(
            <h5 key={key++} className="plan-meal-heading">{text}</h5>
          )
        } else {
          elements.push(
            <h4 key={key++} className="plan-section-heading">{text}</h4>
          )
        }
        return
      }

      if (isMealLine(trimmed)) {
        const colonIdx = trimmed.indexOf(':')
        const mealType = trimmed.substring(0, colonIdx).trim()
        const mealContent = trimmed.substring(colonIdx + 1).trim()
        elements.push(
          <div key={key++} className="plan-meal-row">
            <span className="plan-meal-label">{mealType}:</span>
            <span className="plan-meal-content">{mealContent}</span>
          </div>
        )
        return
      }
      ...
      if (isBullet(trimmed)) {
        elements.push(
          <div key={key++} className="plan-bullet-item">
            <span className="plan-bullet-dot">•</span>
      ...
      // Check if it's a label: value line (e.g. "Sets: 3, Reps: 12")
      if (/^[A-Za-z\s]+:\s*.+/.test(trimmed) && !isHeading(trimmed)) {
        const colonIdx = trimmed.indexOf(':')
        const label = trimmed.substring(0, colonIdx).trim()
        const value = trimmed.substring(colonIdx + 1).trim()
        // Only render as label-value if label is short (not a full sentence)
        if (label.length < 30) {
          elements.push(
            <div key={key++} className="plan-detail-row">
      ...
      elements.push(
        <p key={key++} className="plan-paragraph">{cleanLine(trimmed)}</p>
      )
    })
        ...
        <div className="section-card animate">
          ...
          {dietPlanResult && (
            <div className="diet-plan-result section-card animate">
              ...
              <div className="diet-plan-structured">
                {renderDietPlan(dietPlanResult)}
              </div>
