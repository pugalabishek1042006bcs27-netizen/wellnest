...
const WorkoutPlanner = () => {
  ...
  const renderWorkoutPlan = (rawPlan) => {
    ...
    const isHeading = (line) => {
      ...
      return (
        /^\*\*[^*]+\*\*\s*:?\s*$/.test(t) ||
        /^#+\s+/.test(t) ||
        /^(monday|tuesday|wednesday|thursday|friday|saturday|sunday|day\s*\d+|week\s*\d+|warm[\s-]*up|cool[\s-]*down|main workout|workout plan|notes?|tips?|overview|summary|rest day|circuit|round\s*\d+)\s*:?\s*$/i.test(t) ||
    ...
    lines.forEach((line) => {
      ...
      if (isHeading(trimmed)) {
        ...
        if (/^(monday|tuesday|wednesday|thursday|friday|saturday|sunday|day\s*\d+|week\s*\d+)/i.test(trimmed)) {
          elements.push(<h4 key={key++} className="plan-day-heading">{text}</h4>)
        } else if (/^(warm[\s-]*up|cool[\s-]*down|main workout)/i.test(trimmed)) {
          elements.push(<h5 key={key++} className="plan-meal-heading">{text}</h5>)
        } else {
          elements.push(<h4 key={key++} className="plan-section-heading">{text}</h4>)
        }
        return
      }
      ...
      elements.push(<p key={key++} className="plan-paragraph">{cleanLine(trimmed)}</p>)
    })
          ...
          {workoutPlanResult && (
            <div className="diet-plan-result section-card animate">
              ...
              <div className="diet-plan-structured">
                {renderWorkoutPlan(workoutPlanResult)}
              </div>
